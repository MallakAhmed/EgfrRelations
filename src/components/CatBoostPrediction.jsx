...
function CatBoostPrediction() {
  ...
  const handleFile = async (e) => {
    ...
    try {
      const res = await fetch(`${ML_BASE}/predict_catboost`, {
        method: 'POST',
        body: formData,
      });
      if (!res.ok) {
  ...
  const rowInputId = 'catboost-row-index';

  return (
    <div className="p-6 text-slate-100 min-w-0">
      <h2 className="text-xl font-bold mb-2 text-white">CatBoost ML Prediction</h2>
      <p className="text-xs text-slate-400 mb-4 max-w-2xl">
        Upload a CSV (e.g. df_opt_val). Predictions are computed for every row; charts use the selected row&apos;s age, sex, and creatinine (default 1.0 if absent).
      </p>
      ...
      {loading && <div className="text-cyan-300 text-sm mb-3">Loading and predicting…</div>}

      {data.length > 0 && (
        <>
          <div className="rounded-xl border border-cyan-500/20 bg-slate-900/40 p-4 mb-6">
            <h3 className="text-sm font-semibold text-cyan-200 mb-2">Selected row</h3>
            <div className="flex flex-wrap items-center gap-4 mb-3">
              ...
              <div>
                <span className="text-slate-400 text-xs">CatBoost eGFR: </span>
                <span className="text-cyan-300 font-mono text-lg font-bold">
                  {selected.egfr_pred != null && selected.egfr_pred !== ''
                    ? Number(selected.egfr_pred).toFixed(2)
                    : '—'}
                </span>
              </div>
            </div>
            <p className="text-[11px] text-slate-500">
              Charts below compare six creatinine-based equations across age and creatinine; the purple line is this row&apos;s CatBoost prediction.
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-600/30 overflow-hidden">
            <div className="px-3 py-2 bg-slate-800/60 text-xs text-slate-400 flex flex-wrap justify-between gap-2">
              <span>
                Preview: first <strong className="text-slate-200">{previewRows.length}</strong> of{' '}
                <strong className="text-slate-200">{data.length}</strong> rows (full file was predicted). Click a row or use row index above.
              </span>
            </div>
            ...
            {data.length > ML_TABLE_PREVIEW_ROWS && (
              <p className="text-[11px] text-amber-200/90 px-3 py-2 bg-amber-950/30">
                Rows beyond {ML_TABLE_PREVIEW_ROWS} are not listed here — use the row index field to select them (e.g. {ML_TABLE_PREVIEW_ROWS + 1}).
              </p>
            )}
          </div>
        </>
...
export default CatBoostPrediction;
